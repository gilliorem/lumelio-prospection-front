...
import { Button, FormControl, Tab, Tabs, Typography } from "@mui/material";
...
// import { updateContact, addContact, selectContactsById, getContact } from '../store/contactsSlice';
...
import { selectContactsById } from "../store/contactsSlice";
...
function Contact(props) {
  ...
  const contact = useSelector((state) =>
    selectContactsById(state, routeParams.userId)
  );
  const location = useLocation();

  useEffect(() => {
    if (!contact && location.pathname !== "/utilisateurs/ajouter") {
      // dispatch(getContact(routeParams.contactId));
      setNoContact(true);
    }
  ...
  const schema = yup.object().shape({
    ...
    password:
      location.pathname === "/utilisateurs/ajouter"
        ? yup
            .string()
  ...
  if (noContact) {
    return (
      ...
      // 	</Typography>
      // 	<Button className="mt-24" component={Link} variant="outlined" to="/utilisateurs" color="inherit">
